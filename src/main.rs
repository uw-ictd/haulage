...
async fn main() {
    ...
    let interface_name_match = |iface: &pnet_datalink::NetworkInterface| iface.name == interface_name;
    ...
    let interface_log = root_log.new(
        o!("interface" => String::from(&interface.name[..])));

    for _ in 0..100 {
        match rx.next() {
            Ok(packet) => {
                //slog::info!(log, "Got a packet {:?}", packet);
                let fivetuple = packet_parser::parse_ethernet(
